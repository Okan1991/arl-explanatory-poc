...
def process_fhir_observation(fhir_obs: Dict[str, Any], kb: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if expected_unit and expected_unit != unit:
        return {"observation_code": loinc_code, "status": "rejected", "reason": f"Unit mismatch (expected {expected_unit}, got {unit})."}
...
def find_convergence(explanations: List[Dict[str, Any]], marker_set: List[str], min_support: int = 2) -> Optional[Dict[str, Any]]:
    relevant = [e for e in explanations if e and e.get("status") == "ok" and e.get("observation_code") in marker_set and e.get("category") != "normal"]
...
# Load files
with open('cvd_knowledge.json', 'r') as f: kb = json.load(f)
with open('cvd_observations.json', 'r') as f: observations = json.load(f)
